...
std::string eosio::public_key_to_string(const public_key& key) {
    ...
    } else if (key.index() == key_type::gm) {
        return key_to_string(key, "GM", "PUB_GM_");
    } else {
...
public_key eosio::public_key_from_string(std::string_view s) {
    ...
    if (s.substr(0, 3) == "EOS") {
        ...
        return string_to_key<public_key>(s.substr(7), key_type::gm, "GM");
    } else {
...
std::string eosio::private_key_to_string(const private_key& private_key) {
    ...
    else if (private_key.index() == key_type::gm)
        return key_to_string(private_key, "GM", "PVT_GM_");
    else {
...
private_key eosio::private_key_from_string(std::string_view s) {
    if (s.substr(0, 7) == "PVT_K1_")
        ...
        return string_to_key<private_key>(s.substr(7), key_type::r1, "GM");
    else if (s.substr(0, 7) == "PVT_GM_")
        return string_to_key<private_key>(s.substr(7), key_type::gm, "R1");
...
std::string eosio::signature_to_string(const eosio::signature& signature) {
    ...
    else if (signature.index() == key_type::gm)
        return key_to_string(signature, "GM", "SIG_GM_");
    else {
...
signature eosio::signature_from_string(std::string_view s) {
    ...
    else if (s.size() >= 7 && s.substr(0, 7) == "SIG_GM_")
        return string_to_key<signature>(s.substr(7), key_type::gm, "GM");
    else {
       check(false, convert_json_error(eosio::from_json_error::expected_signature));
       __builtin_unreachable();
    }
